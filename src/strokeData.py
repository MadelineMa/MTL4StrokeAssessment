...
if __name__ == '__main__':
    ...
    if os.name == 'posix':
        train_fn = '../GN-data/train.csv'
        valid_fn = '../GN-data/valid.csv'
        test_fn = '../GN-data/test.csv'
    else:
        train_fn = '..\\GN-data\\train.csv'
        valid_fn = '..\\GN-data\\valid.csv'
        test_fn = '..\\GN-data\\test.csv'
    dft= dft.drop(dft[(dft['档案年度']==2019) & (dft['风险评级']==5)].index)
    dft = dft.drop(dft[(dft['风险评级']==0)].index)
    dft.drop(['身份证号', '档案年度'], axis=1, inplace=True)
    dft = dft.reset_index(drop=True)

    col_name=dft.columns.tolist()
    col_name.insert(7,'卒中')
    dft = dft.reindex(columns=col_name)
    dft['卒中'].loc[dft[dft['风险评级']==5].index] = 1
    dft['卒中'] = dft['卒中'].fillna(0).astype(int)

    dft['风险评级'] = dft['风险评级'].apply(lambda x: x - 1)
    # dft['风险评级'] = dft['风险评级'].apply(lambda x: 2 if x == 4 else x) ## MLiao: uncomment if treat `stroke` as `high` risk.
    dft['风险评级'] = dft['风险评级'].apply(lambda x: 3 if x == 4 else x)
    # dft['风险评级'] = dft['风险评级'].apply(lambda x: 1 if x==5 else 0)
    dft = dft.reset_index(drop=True)
    
    #根据feature importance 的顺序，挑选top 20的特征进行实验
